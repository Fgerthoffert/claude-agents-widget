 ...
 * it when the agent stopped, so the same number reads as how long nothing has happened. The two
 * mean opposite things, so each gets its own icon (ADR-0008).
 *
...
export const describeAge = (session: Session, nowMs: number): AgeDisplay | null => {
  ...
  if (session.state === 'working') {
    return { kind: 'active', icon: '⏱', text, label: `working for ${text}` };
  }

  const verb = session.state === 'needs_input' ? 'waiting' : 'inactive';

  return { kind: 'inactive', icon: '⏳', text, label: `${verb} for ${text}` };
};
